...
namespace GoMoPhoConsole
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            var byteSearch = ToBytes(hexSearch);
            string folder = null;
            if (args.Length == 0)
            {
            ...
            Console.WriteLine($"Finished. Processed {count} videos for {imageFiles.Length} images files. Press any key to exit");
            Console.ReadKey();
        }
        ...
        public static byte[] ToBytes(string hexValues)
        {
            ...
            byte[] characters = new byte[hexValuesSplit.Length];
